...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let [ref mut way_bar, ref mut way_menu, ref mut way_work] = windows[..] else {
        panic!("Error getting wayland handles");
    };

    let run_dir = env::var("XDG_RUNTIME_DIR");
    let inst_dir = env::var("HYPRLAND_INSTANCE_SIGNATURE");
    let forge = Forge::new(way_work.get_handler());
    // let bar_forge = Forge::new(way_bar.get_handler());
    ...
    enchant_spells::<Box<dyn FnMut(Arc<RwLock<Box<dyn ForeignController>>>)>>(
        windows,
        vec![None, Some(Arc::new(RwLock::new(state))), None],
        vec![
            None,
            Some(Box::new(
